...
os.system("minimap2 -a -x map-ont -t 16 %s %s | samtools view -bS -F 4 - | samtools sort -o %s/%s.bam -" %(ref_file,fastq_path,result_path,sample_name))
...
os.system(f'python {make_depth_mask_path}/make_depth_mask.py {result_path}/{sample_name}.bam {result_path}/{sample_name}.coverage_mask.txt {ref_file} 15')
