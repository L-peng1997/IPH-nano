...
from G_UI.weizhi_ctpj import Ui_Dialog as WZctpj_UI
...
class Ui_MainWindow(QObject):
    ...
    def setupUi(self, MainWindow):
        ...
        self.typeTree.setMinimumWidth(250)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.typeTree.topLevelItem(0).setText(0, self._translate("MainWindow", "新冠病毒"))
        self.typeTree.topLevelItem(0).child(0).setText(0, self._translate("MainWindow", "测序"))
        ...
        self.typeTree.topLevelItem(1).setText(0, self._translate("MainWindow", "诺如病毒"))
        self.typeTree.topLevelItem(2).setText(0, self._translate("MainWindow", "未知病原"))
        ...
        self.typeTree.topLevelItem(3).setText(0, self._translate("MainWindow", "流感病毒"))
        self.typeTree.topLevelItem(4).setText(0, self._translate("MainWindow", "宏基因组"))
        self.typeTree.topLevelItem(4).child(0).setText(0, self._translate("MainWindow", "查找参考序列"))
        self.typeTree.topLevelItem(4).child(1).setText(0, self._translate("MainWindow", "序列拼接"))
        self.typeTree.topLevelItem(4).child(2).setText(0, self._translate("MainWindow", "从头拼接"))
        self.typeTree.topLevelItem(5).setText(0, self._translate("MainWindow", "二代测序"))
        self.typeTree.topLevelItem(5).child(0).setText(0, self._translate("MainWindow", "数据质控"))
        self.typeTree.topLevelItem(5).child(1).setText(0, self._translate("MainWindow", "序列拼接(基于参考序列)"))
        # self.typeTree.topLevelItem(5).child(2).setText(0, self._translate("MainWindow", "序列提取"))
        self.typeTree.topLevelItem(5).child(2).setText(0, self._translate("MainWindow", "从头拼接"))
    ...
    def add_param(self):
        ...
        if current_tree in ['新冠病毒', '测序', '未知病原']:
            ...
            if self.typeTree.currentItem().parent().text(0) == '宏基因组':
                try:
    ...
    def run_(self, signal):
        ...
        if signal == 1:
            ...
            # 运行程序
            if self.typeTree.currentItem().text(0) in ['新冠病毒', '测序']:
            ...
            elif self.typeTree.currentItem().text(0) == '未知病原':
            ...
            elif self.typeTree.currentItem().text(0) == '流感病毒':
                self.do_liugan(self.params)
            elif self.typeTree.currentItem().text(0) in ['查找参考序列', '宏基因组', '序列拼接']:
    ...
    def judge_path(self, file_path):
        ...
        if os.path.exists(file_path):
            ...
            try:
                ...
                elif self.typeTree.currentItem().text(0) in ['未知病原', '流感病毒', '宏基因组']:
    ...
    def tree_click(self, item):
        ...
        if parent_ == '未知病原' and item.text(0) == '可视化分析':
            data = {
                'task_type': '',
                'task_name': '',
                'sample_list': '',
    ...
    def open_file(self, table):
        ...
        if button:
            ...
            path = path[0] if path else '.'
            # work_file = path
            if '正在运行' in status or task_type not in ['新冠病毒', '新冠病毒-测序']:
                work_file = path
            else:
